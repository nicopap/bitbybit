...
- The methods generated by the `bitfield!` macro — the `with_*` setters, getters, `build()`,
  `builder()`, `new_with_raw_value()`, `raw_value()` and the introspection `mask_*()` helpers — are
  now marked `#[must_use]`. Callers that discard these results will get an `unused_must_use`
  warning, which becomes a hard error under `-D warnings`; this is a minor-version-level breaking
  change for such projects. (@nicopap)
